 ...
 *
 * Compile:
 *   gcc -Wall -pedantic -o sparometer sparometer.c
 *
 */
...
void setdatetime(char **date, char **time)  {
	struct timeval now;
	time_t curtime;
	static char s_date[7];
	static char s_time[7];

	gettimeofday(&now, NULL);
	curtime = now.tv_sec;

	strftime(s_date, sizeof(s_date), "%y%m%d", localtime(&curtime));
	*date = (char *)s_date;
	strftime(s_time, sizeof(s_time), "%H%M%S", localtime(&curtime));
	*time = (char *)s_time;
}
	...
	while (command_serial[i].name != NULL) {
		...
		if (command_serial[i].short_option != 0) {
			...
			if (DEBUG && map_short_longidx[command_serial[i].short_option]) {
				fprintf(stderr, "PROGRAM ERROR, option argument '%c' occour twice\n", command_serial[i].short_option);
				exit(1);
			}
...
void printbuf(void *buf, int count, char *extra, int local_verbose) {
	if (count) {
...
void show_help(void) {
	...
	while (command_serial[i].name) {
		...
		if (c->command_data && verbose) {
			...
			printbuf(
				(char *)*(c->command_data),
				strlen(*(c->command_data)),
				c->has_arg ? *(char **)c->command_arg : NULL,
				1);
		}
		if (c->mode_baud != none) {
			printf("\tMode: ");
...
int wp(int fd, const void *buf) {
	...
	while (try--) {
		if (writecom(fd, buf)) {
			if (verbose) {
				printf("\nWrite: ");
				printbuf((char *)buf, strlen(buf), NULL, 1);
			}
...
int sendreceive(int fd, struct command_serial_t *cmd ) {
	char wbuffer[1024];
	char rbuffer[1024];
	enum error_response_t resp;
	int num = 0;
	...
	/* Merge command argument with command */
	...
	while (try--) {
		if (writecom(fd, &wbuffer)) {
			if (verbose) {
				printf("\nWrite: ");
				printbuf((char *)&wbuffer, strlen(wbuffer), NULL, 1);
			}
			num = readwait(fd, &rbuffer, sizeof(rbuffer));
			if (num) {
...
int exec_chain_list(int fd, struct command_que_t *head) {
	...
	while (head) {
		++count;
		sendreceive(fd, &command_serial[head->idx]);
		/*
...
int processdevice(char *device) {
	...
	if ((fd = open(device, O_RDWR | O_NOCTTY )) > 0) {
		...
		while (num) {
			num = readwait(fd, &buffer, sizeof(buffer));
			if (num) printbuf(&buffer, num, NULL, verbose);
		}
